...
public interface SanPhamChiTietADMRepository extends JpaRepository<SanPhamChiTiet, Integer> {
   ...
   """, nativeQuery = true)
    ...
    // ===== LOW-STOCK: sản phẩm sắp hết hàng (tồn kho < threshold) =====
    ...
    @Query("""
       ...
       WHERE spct.trangThai = true
         AND (spct.soLuong IS NULL OR spct.soLuong < :threshold)
       ORDER BY spct.soLuong ASC
   """)
    List<SanPhamChiTiet> findLowStockItems(@Param("threshold") int threshold);


    @Query("""
       SELECT COUNT(spct) FROM SanPhamChiTiet spct
       WHERE spct.trangThai = true
         AND (spct.soLuong IS NULL OR spct.soLuong < :threshold)
   """)
